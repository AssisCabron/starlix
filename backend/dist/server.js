...
// HWID Reset Endpoint
app.post('/api/user/hwid-reset', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader)
...
function generateLicenseKey(amount) {
...
async function checkPendingPayments() {
    ...
    try {
        ...
        const { data: pendingPayments, error } = await Promise.race([
            fetchPromise,
            new Promise((_, reject) => setTimeout(() => reject(new Error('DB Query Timeout')), 10000))
        ]);
        ...
        if (pendingPayments && pendingPayments.length > 0) {
            ...
            for (const payment of pendingPayments) {
                try {
                    ...
                    const response = await axios_1.default.get(`${ASAAS_API_URL}/payments/${payment.asaas_id}`, {
        ...
        if (resellerPurchases && resellerPurchases.length > 0) {
            ...
            for (const purchase of resellerPurchases) {
                try {
                    const response = await axios_1.default.get(`${ASAAS_API_URL}/payments/${purchase.payment_id}`, {
                        headers: { access_token: ASAAS_API_KEY }
                    });
                    const asaasStatus = response.data.status;
...
app.post('/api/payments/checkout', async (req, res) => {
    ...
    try {
        ...
        if (!customerId) {
            ...
            const customerResponse = await axios_1.default.post(`${ASAAS_API_URL}/customers`, {
            ...
            try {
                await axios_1.default.post(`${ASAAS_API_URL}/customers/${customerId}`, {
                    name: name,
                    cpfCnpj: cpfCnpj,
                    phone: phone,
                    mobilePhone: phone
        ...
        const response = await axios_1.default.post(`${ASAAS_API_URL}/payments`, {
            ...paymentPayload
        }, {
            headers: { access_token: ASAAS_API_KEY }
        });
        ...
        if (returnedStatus === 'PENDING' && type === 'PIX') {
            try {
                const pixResponse = await axios_1.default.get(`${ASAAS_API_URL}/payments/${response.data.id}/pixQrCode`, {
                    headers: { access_token: ASAAS_API_KEY }
                });
                pixData = {
...
app.get('/api/payments/status/:asaas_id', async (req, res) => {
...
app.get('/api/reseller/wholesale-prices', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader)
        return res.status(401).json({ error: 'Missing Token' });
    const token = authHeader.replace('Bearer ', '');
...
app.post('/api/reseller/purchase-keys', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader)
        return res.status(401).json({ error: 'Missing Token' });
    const token = authHeader.replace('Bearer ', '');
    ...
    try {
        ...
        if (customerId) {
            try {
                await axios_1.default.get(`${ASAAS_API_URL}/customers/${customerId}`, {
                    headers: { access_token: ASAAS_API_KEY }
                });
        ...
        const paymentResponse = await axios_1.default.post(`${ASAAS_API_URL}/payments`, paymentPayload, {
            headers: { access_token: ASAAS_API_KEY }
        });
        ...
        if (billingType === 'PIX') {
            try {
                const pixResponse = await axios_1.default.get(`${ASAAS_API_URL}/payments/${paymentId}/pixQrCode`, {
                    headers: { access_token: ASAAS_API_KEY }
                });
                pixData = {
                    encodedImage: pixResponse.data.encodedImage,
...
app.post('/api/payments/redeem-key', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader)
        return res.status(401).json({ error: 'Missing Token' });
    const token = authHeader.replace('Bearer ', '');
